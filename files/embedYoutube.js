		...
		'constructor': function() {
			common.addFilter('embedYoutube', function(entries, skelton) {
				...
				entries = common.replaceURLs(entries, function(url) {
					...
					if (matches) {
						return '<iframe type="text/html" src="ht'+'tp://www.youtube.com/embed/'+matches[1]+'" class="extension-embed-youtube-video" frameborder="0" allowfullscreen />';
					}
				});
				// 再生リスト
				entries = common.replaceURLs(entries, function(url) {
					...
					if (matches) {
						return '<iframe src="ht'+'tp://www.youtube.com/embed/videoseries?list='+matches[1]+'" class="extension-embed-youtube-playlist" frameborder="0" allowfullscreen></iframe>';
					}
				});
				return entries;
			});
